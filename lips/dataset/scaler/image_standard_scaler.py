...
class StandardScalerPerChannel():
    ...
    X - mean(X) / std(X)
    """
    def __init__(self,channelIndex_x=1,channelIndex_y=1):
        self.channelIndex_x = channelIndex_x
        self.channelIndex_y = channelIndex_y
        self._m_x = []
        self._m_y = []
        self._std_x = []
        self._std_y = []
    ...
    def fit(self, x, y):
        for channel in range(x.shape[self.channelIndex_x]):
            x_channel_axis=x.take(channel,axis=self.channelIndex_x)    
        ...
        for channel in range(y.shape[self.channelIndex_y]):    
            y_channel_axis=y.take(channel,axis=self.channelIndex_y)
    ...
    def transform(self, x, y):
        shapeBychannel_x=x.shape[:self.channelIndex_x]+x.shape[self.channelIndex_x+1:]
        ...
        for channel in range(x.shape[self.channelIndex_x]):
            ...
            transformed_x[:,channel,::]=x_channel.reshape(shapeBychannel_x)

        shapeBychannel_y=y.shape[:self.channelIndex_y]+y.shape[self.channelIndex_y+1:]
        ...
        for channel in range(y.shape[self.channelIndex_y]):
            ...
            transformed_y[:,channel,::]=y_channel.reshape(shapeBychannel_y)
