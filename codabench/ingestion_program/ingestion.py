...
def _check_simulator_method(model_cls: ModuleType):
    ...
    if not issubclass(model_cls, AugmentedSimulator):
        raise ModelApiError("Your simulator class does not inherit from AugmentedSimulator.")
...
def _import_simulator() -> ModuleType:
    """Import user-provided simulator code"""
    model_cls = importlib.import_module("my_augmented_simulator").BenchmarkedSimulator
    _check_simulator_method(model_cls)
    logger.info("Provided implementation of AugmentedSimulator is compliant with the interface.")
    return model_cls


def init_simulator(args: Namespace) -> Tuple[AugmentedSimulator, Dict]:
    ...
    simulator = _import_simulator()(**kwargs, **vars)
...
def create_simulator(args: Namespace, benchmark: Benchmark) -> AugmentedSimulator:
    """Create the simulator using the benchmark and provided configuration"""
    simulator, simulator_config = init_simulator(args)
    pre_trained_path = args.submission_dir / simulator_config["trained_model_path"]
...
def main():
    ...
    ts = _init_timer()
    simulator: AugmentedSimulator = create_simulator(args, benchmark)
